...
class MovieSerializer(serializers.ModelSerializer):
    movie_file_url = serializers.SerializerMethodField()
    poster_image_url = serializers.SerializerMethodField()

    class Meta:
        model = Movie
        fields = ['title', 'trailer_url', 'movie_file_url', 'poster_image_url']

    def get_movie_file_url(self, obj):
        request = self.context.get('request')
        return request.build_absolute_uri(obj.movie_file.url) if obj.movie_file else None

    def get_poster_image_url(self, obj):
        request = self.context.get('request')
        return request.build_absolute_uri(obj.poster_image.url) if obj.poster_image else None
