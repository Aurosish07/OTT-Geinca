...
class MovieViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        language = self.request.query_params.get('language', None)  # Get language from query params

        # Debugging: print or log the value of the language parameter
        print(f"Filtering movies by language: {language}")

        if language:
            # Check if filtering by language is working
            queryset = queryset.filter(language__iexact=language)  # Case-insensitive match for language
    ...
    def get_queryset(self):
        ...
        return queryset
class UpcomingMoviesViewSet(viewsets.ModelViewSet):
    serializer_class = MovieSerializer
    permission_classes = [IsAuthenticatedOrReadOnly]

    def get_queryset(self):
        now = timezone.now()
        queryset = Movie.objects.filter(release_date__gte=now, is_upcoming=True, status='published')
        
        # Debug: Print the resulting queryset
        print(queryset)
...
class MovieViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        # This will be called when accessing /api/movies/<id>/
        return super().retrieve(request, *args, **kwargs)

    @action(detail=False, methods=['get'], url_path='(?P<genre>[^/.]+)')
    def get_movies_by_genre(self, request, genre=None):
        movies = self.queryset.filter(genre__iexact=genre)  # Filter movies by genre
...
class LiveNewsViewSet(viewsets.ModelViewSet):
    ...
    # Optionally, if you want to filter more in the queryset, you can define it here
    def get_queryset(self):
        return LiveNews.objects.filter(is_active=True) 
...
class AdViewSet(viewsets.ModelViewSet):
    ...
    # Allow unrestricted access by setting permission classes
    permission_classes = []  
